...
새 transport는 `contract_revision = "orchestration-2026-08-07-r3"`를 필수로 하고
...
`contract_revision`과 기존 내부 인증 binding을 가진다. response는 CallRuntime
snapshot 하나와 AgentRuntime 입력 하나를 전달하며 mode 또는 transition
snapshot을 포함하지 않는다.
...
```text
BootstrapOrchestrationResponse
├── contractRevision = "orchestration-2026-08-07-r3"
...
## CallRuntime snapshot

`call_runtime`은 통화 관련 설정 전체를 한 번만 전달한다.
...
인터럽션 또는 통화 timeout 필드를 두지 않는다. LLMWorker는 optional이 아니며
전역 LLM field를 제공하지 않는다.
...
- CallRuntime이 AgentRuntime 사이에 중복되지 않음
- revision 누락·불일치 거부
