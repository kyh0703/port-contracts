...
test("npm release metadata is pinned to 7.8.0", () => {
  const packageJson = JSON.parse(read("package.json"));
  const packageLock = JSON.parse(read("package-lock.json"));

  assert.equal(packageJson.version, "7.8.0");
  assert.equal(packageLock.version, "7.8.0");
  assert.equal(packageLock.packages[""].version, "7.8.0");
});
